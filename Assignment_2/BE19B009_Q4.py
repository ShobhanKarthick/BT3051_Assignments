    ...
    while i in range(len(infix) + 1):
        ...
        try:
            while not infix[i] in operators:  # Filtering only numbers
                if i < len(infix):
                    # Forming the number by conactenating digits
                    number += infix[i]
                    i += 1
                else:
                    break
            ...
            if number != "":
                ...
                postfixStack.append(
                    int(number)
                )  # Push the number onto the postfix stack
            ...
            if infix[i] in operators:
                ...
                if len(postfixStack) == 0:
                    postfixStack.append(
                        infix[i]
                    )  # If postfix stack is empty, push operator directly
                    ...
                    infix[i] == "(" or infix[i] == "^"
                ):  # ^ and ( have the highest precedence hence push directly
                    operatorStack.append(infix[i])

                elif (
                    infix[i] == "*" or infix[i] == "/"
                ):  # Next in precedence is * and /, so push to the right place
                    ...
                    infix[i] == "-" or infix[i] == "+"
                ):  # Lowest precedence is + and -, so push to the right place
                    if len(operatorStack) != 0:
                        try:
                            while (
                                operatorStack[-1] == "+"
                    ...
                    infix[i] == ")"
                ):  # On encountering ), pop from operator stack until (  and push to postfix stack
                    if len(operatorStack) != 0:
                        try:
